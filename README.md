...
巴菲特的“别人恐惧我贪婪，别人贪婪我恐惧”，我们将其绝对数学化。
- **数据源**：Alternative.me 恐慌贪婪指数 (0-100)。
- **计算公式**：`Sentiment Score = (100 - FG指数) / 50.0`
- **底层原理**：
  - FG指数默认情况下 50 为中性。` (100 - 50) / 50.0 = 1.0`（正常买入基准分）。
  - 极端恐慌下（比如恐慌到了 10），得分飙升：`(100 - 10) / 50.0 = 1.8`。逆向大幅度增加资金！
  - 极度贪婪时（比如狂热到了 90），得分为：`(100 - 90) / 50.0 = 0.2`。压缩买入欲望，规避随时可能发生的踩踏。

### 因子 3: 趋势因子 (Trend) - 权重比例：30%
即使便宜+恐慌，如果不顺势，也可能陷入长达半年的长期阴跌（所谓的“不要接下落的飞刀”）。
- **数据源**：Yahoo Finance 获取 QQQ 当前价格与 200 日定投生命线（即半年线），算出乖离率百分比。
- **计算逻辑（梯队赋分）**：
  - **严重超买 (乖离率 > 15%)**：得分强制腰斩为 `0.5`。远离均线过远必有引力场，此处发生均值回归跳水回落的概率极其大，所以大力压制多头乘数。
  - **多头排列 (0 < 乖离率 <= 15%)**：价格在半管线之上，呈上风。由于顺大趋势向上，得分奖励提升至 `1.2`，在右侧稳步加权。
  - **空头防守 (乖离率 <= 0)**：价格跌破半年生命线，“飞刀”正在下落或处于震荡盘整期，存在继续探底风险，只打出防守分 `0.8`。
