...
- **计算公式**（分段平滑防守）：
  - 当 `PE分位 < 70%` 时：`Valuation Score = 2.0 * (1.0 - PE百分位)`
  - 当 `70% <= PE分位 <= 80%` 时：`Valuation Score = 6.0 * (0.8 - PE百分位)` (加速滑落至0)
  - 当 `PE分位 > 80%` 时：`Valuation Score = 0.0` (彻底切断买入)
- **底层原理**：
  - 在估值处于历史合理及低估阶段（<70%），线性赋予买入权重。例如 `PE分位 = 50%` 时，提供客观中立的正常 1倍基础权重。
  - 当进入偏高估值区域（突破70%），得分为 `0.6`。为了防止长期在泡沫区接盘，在走向 80% 的区间里，估值分数被极其严厉地削减。
  - 当 `PE分位 >= 80%` 时，不再被动等待泡沫到达 100% 的极值，而是提前强行将本项因子的得分**直接清零 (0.0)**，坚决对高位风险执行右侧防守。

### 因子 2: 市场情绪因子 (Sentiment) - 权重比例：30%
巴菲特的“别人恐惧我贪婪”，我们将其绝对数学化。
- **数据源**：Yahoo Finance 的 CBOE 纳斯达克100期权隐含波动率指数 (^VXN)。
- **计算公式**：
  - 当 `VXN < 20` 时：`Sentiment Score = (VXN - 10) / 10.0` （并限制最低为 0）
  - 当 `VXN >= 20` 时：`Sentiment Score = VXN / 20.0` （限制最高为 2.0 倍）
- **底层原理**：
  ...
  - 极度贪婪时（例如波动率跌至 10 或更低），得分为保底惩罚分 `0.0`。剥夺情绪买入权重，防范黑天鹅踩踏。
