...
def load_existing_js(output_path):
    """加载现有的 backtest_data.js"""
    ...
    # 提取 JSON 部分
    match = re.search(r'const BACKTEST_DATA = ({.*?});\nif', content, re.DOTALL)
    if match:
        try:
...
def export_to_js(results, output_path):
    """将回测结果增量更新或导出为 JS 格式文件"""
    existing_data = load_existing_js(output_path)
    
    export_data = {}
    for name, df in results.items():
        # 如果已有数据，则获取最新日期
        existing_records = existing_data.get(name, [])
        last_date = None
        if existing_records:
            last_date = existing_records[-1]['date']
        ...
        if existing_records:
            export_data[name] = existing_records + new_records
            print(f"  {name}: 原有 {len(existing_records)} 条，新增 {len(new_records)} 条记录")
        else:
            export_data[name] = new_records
            print(f"  {name}: 全新生成 {len(new_records)} 条记录")
